...
our ($refGenome, $output, $folder_temp, $folderCheckedForStat, $path_R_Scripts, $path_SeqrefGenome) = ("empty", "empty", "empty", "empty", "empty", "empty");  # The reference genome to use; The path for saving the result; The path for saving the temporary files; The path for saving the files with enough mutations for calculating the statistics; The path to R scripts; The path to the fasta reference sequences
...
GetOptions('verbose|v'=>\$verbose, 'help|h'=>\$help, 'man|m'=>\$man, 'refGenome=s'=>\$refGenome, 'outfile|o=s' => \$output, 'pathTemporary|temp=s' => \$folder_temp, 'pathFolderCheck|check=s' => \$folderCheckedForStat, 'pathRscript=s' => \$path_R_Scripts, 'pathSeqRefGenome=s' => \$path_SeqrefGenome, 'poolData' => \$poolData, 'reportSample' => \$oneReportPerSample) or pod2usage(2);
...
sub CheckFlags
{
	...
	# If no temp folder for the checked variants is specified write the result in the current paht
	if($folderCheckedForStat eq "empty") { $folderCheckedForStat = "$pwd/folder_checked_$filename"; }
...
=head1 SYNOPSIS
  ...
  Arguments:
        ...
        -check     --pathFolderCheck             the path for saving the variants with enough mutations
